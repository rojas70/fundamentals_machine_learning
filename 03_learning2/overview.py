...
Files: 
    ...
    - names.train, names.dev, names.test: files to train/validate/test
...
Goal:
    1. Learn a predictor based on:
        - Available train and validation sets
        - Your customized feature extractor
...
Feature Extractor:
    ...
    - Instead of using a standard dictionary, try a default dict(float).
...
1. Making sense of the data
    ...
    - You begin with a null feature extractor
    - Start by running the empty main_simple.py as a simple sanity check

2. Beging by creating a simple feature extractor
