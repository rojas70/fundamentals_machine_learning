...
def increment(d1, scale, d2):
    ...
    for f, v in list(d2.items()):
        d1[f] = d1.get(f, 0) + v * scale # w = w + phi(x)*y*eta*-1
...
def outputWeights(weights, path):
    ...
    for f, v in sorted(list(weights.items()), key=lambda f_v: -f_v[1]): # sort by value, large to small key https://medium.com/analytics-vidhya/sorted-function-using-key-parameter-in-python-7aa9b8cebfb6
...
def generateClusteringExamples(numExamples, numWordsPerTopic, numFillerWords):
    ...
    The actual review consists of 2 sentiment words, 4 topic words and 1 filler word, for example:
        ...
        good:1 great:1 plot1:2 plot7:1 plot9:1 plot11:1 filler0:1 filler10:1
