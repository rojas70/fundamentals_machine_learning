...
def learnPredictor(trainExamples, testExamples, featureExtractor, numIters, eta):
    ...
    def predict(x):
        '''
        Classifier predictor is the sign(\phi(x) \cdot w)
        '''
    ...
    for i in range(numIters):
        ...
        for item in trainExamples:
            ...
            # gradLoss: error updates for wrong classifications
            if temp < 1:
                # w = w  - eta*-y*phi for sparse vectors. Also note that w is updated to match the feature space. 
                increment(weights,-eta*-y,phi) # hinge loss: minimize if margin is less than 1
       
        # Report error progress
        print("Iteration:%s, Training error:%s, Test error:%s"%(i,
                                                                evaluatePredictor(trainExamples,predict),
                                                                evaluatePredictor(testExamples,predict)))
    # END_YOUR_CODE
    return weights
